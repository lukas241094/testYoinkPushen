...
public class DensityCalculator implements
	...
	public Double calculate(Coord currentCoord, Set<Molecule> molecules) {
		...
		double density2 = 0;
			...
			density2 = loopOverEveryAtomStream(currentCoord, molecules);
	...
	private double loopOverEveryAtom(Coord currentCoord, Set<Molecule> molecules) {
		...
		System.out.println("Calculation time Imp "+ (time2-time1));
		return density;
	}
	
	private double loopOverEveryAtomStream(Coord currentCoord, Set<Molecule> molecules) {
		long time1 = java.lang.System.nanoTime();
		double density = 0;
		for ( Molecule molecule : molecules){
			
			density += molecule.getAtoms().stream().parallel()
...
//				 .sum();
		...
		long time2 = java.lang.System.nanoTime();
		System.out.println("Calculation time Str "+ (time2-time1));
		return density;
	}
