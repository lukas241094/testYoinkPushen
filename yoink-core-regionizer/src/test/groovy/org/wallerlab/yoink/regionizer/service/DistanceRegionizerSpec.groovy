...
class DistanceRegionizerSpec extends Specification{
	def "test method regionize(Map<Region.Name, Region> regions,Map<JobParameter, Object> parameters)"(){
		...
		m1.getAtoms()>>[a1]
		m1.getDistanceToCore()>>(double)   1.0
		def a2=Mock(Atom)
		def m2=Mock(Molecule)
		m2.getAtoms()>>[a2]
		m2.getDistanceToCore()>>(double)   2.0
		def a3=Mock(Atom)
		def m3=Mock(Molecule)
		m3.getAtoms()>>[a3]
		m3.getDistanceToCore()>>(double)   3.0
		
		def m4 = new SimpleMolecule(1,[a3])
		def m5 = new SimpleMolecule(1,[a3])
		def m6 = new SimpleMolecule(1,[a3])
		
		region2.getMolecules()>>[m1, m2, m3, m4,m5,m6]
		...
		distanceCalculator.calculate(_,m4)>>(double)3.0
		distanceCalculator.calculate(_,m5)>>(double)5.0
		distanceCalculator.calculate(_,m6)>>(double)1.0
		...
		//regionizer.regionizer = "FUNCTIONAL"

		then:"the new distanceRegionizer is executable and gets right results"
		regionizer.regionize(regions,parameters)
		regions.size()==5
		regions.get(Region.Name.QM_ADAPTIVE).getSize()==3
		regions.get(Region.Name.BUFFER).getSize()==2
	}
}
